...
@section('title', __('admin.Add package'))
@section('page_title', __('admin.Add package'))
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-sm">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">{{ __('admin.Package Information') }}</h2>
        </div>

        <form action="{{ route('admin.pricing.store') }}" method="POST" class="p-6 space-y-6" id="pricing-form">
            @csrf
            ...
            <div>
                ...
                <input type="number" name="price" id="price" value="{{ old('price') }}" required step="0.01" min="0" class="w-full md:w-48 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('price') border-red-500 @enderror">
                @error('price')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-6">
                <div class="flex items-center">
                    ...
                    <input type="checkbox" name="is_popular" id="is_popular" value="1" {{ old('is_popular') ? 'checked' : '' }} class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
